...
export function TableToolbar({
    ...
    const resetButton = onReset ? (
        <button
            onClick={onReset}
            className="h-11 w-11 sm:h-10 sm:w-10 shrink-0 inline-flex items-center justify-center bg-white dark:bg-slate-900  dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl text-slate-500 transition-all shadow-sm touch-manipulation"
            title="Reset filters"
        >
            <RefreshCw size={16} />
        </button>
    ...
    return (
        <div
            className={cn(
                isStacked
                    ? 'flex flex-col gap-3'
                    : 'flex flex-col xl:flex-row xl:flex-nowrap xl:items-center gap-3',
                className,
            )}
        >
            {searchField}
            ...
            {isStacked ? (
                <div className="flex flex-wrap items-center gap-2 xl:flex-nowrap xl:overflow-x-auto xl:whitespace-nowrap">
                    {filters}
